...
def count_dropped_before_index(df_filt, index, mainCamNdx=0, debug=False):
    index = int(index)
    if index <= 0:
        return 0
    dropX = np.sum(df_filt['Dropped']['droppedFrmX'][0:index] == 1)
    dropYZ = np.sum(df_filt['Dropped']['droppedFrmYZ'][0:index] == 1)
    if debug:
        return[dropYZ, dropX]
    elif mainCamNdx == 0:
        return dropYZ
...
def getMainCam(video_path):
    """
    Get the main camera nickname and video name base from the system copy file.
    Parameters:
        video_path (str): Path to the video file.
    Returns:
        tuple: Main camera nickname and video name base.
    """
    ...
    sysCopyPath = os.path.join(vid_dir, vid_name_base + '_systemdata_copy.yaml')
    if not os.path.isfile(sysCopyPath):
        sysCopyPath = os.path.join(vid_dir, vid_name_base + '_userdata_copy.yaml')
        if not os.path.isfile(sysCopyPath):
            print('No system copy file available for %s\n' % vid_name_base)
            return None, None
        # Read the system configuration YAML file
    with open(sysCopyPath, 'r') as file:
        yaml_content = yaml.safe_load(file)
    for cam_key, cam_config in yaml_content.items():
        if isinstance(cam_config, dict) and cam_config.get('ismaster', False):
            return cam_config.get('nickname', None), vid_name_base, vid_dir
    return None, None
